...
def psl_upload(request):
    if request.method == 'POST':
        try:
            ...
            # 绝对路径
            save_path = '/data/ssl_chat/ssl_model/userfile/psl/'
            full_path = os.path.join(save_path, local_filename)
            # 保存文件
            
            file_content = uploaded_file.read().decode('utf-8',errors='replace') 
            
            if len(file_content.strip()) == 0:  # 使用 strip() 方法去除可能的空白字符（如换行符、空格等）
                print(f"length: {len(file_content.strip())}") 
                return JsonResponse({'code':900,'msg':'file is null!'}) 
            ...
            subprocess.Popen(['python', '/data/ssl_chat/ssl_model/kDataBase/trans_shell.py'] + params)
            print("主进程继续执行")
            return JsonResponse({'code': 200, 'msg': '文件上传成功！已经存储'})
        except Exception as e:
            print(f"遇到错误：{e}")
            return JsonResponse({"code": 12098, "msg": "遇到错误"})
...
def pub_upload(request):
    if request.method == 'POST' and request.FILES['file']:
        try:
            ...
            local_filename = generate_filename(usr.username, file_extension, usr.type)
            # 绝对路径
            save_path = '/data/ssl_chat/ssl_model/userfile/pub/'
            full_path = os.path.join(save_path, local_filename)
            # 保存文件
            file_content = uploaded_file.read().decode('utf-8',errors='replace') 
            ...
            subprocess.Popen(['python', '/data/ssl_chat/ssl_model/kDataBase/trans_pub_shell.py'] + params)
            print("主进程继续执行")
            return JsonResponse({'code': 200, 'msg': '文件上传成功！已经存储'})
        except Exception as e:
            print(f"遇到错误：{e}")
            return JsonResponse({"code": 12097, "msg": "遇到错误"})
...
def psl_upload_records(request):
    if request.method == 'GET':
        try:
            ...
            records = [{
                "name": record.partition_id.partition_nickname,
            ...
            print(f"遇到错误：{e}")
            return JsonResponse({"code": 12096, "msg": "遇到错误"})
...
def success_check(record, filetype):
    ...
    if record.status == 'failure':
        ...
        base_url = "http://djtest0001.cpolar.top"
...
def partition_info(request):
    if request.method == 'GET':
        try:
            ...
            print(f"遇到错误：{e}")
            return JsonResponse({"code": 12095, "msg": "遇到错误"})
...
def psl_share_records(request):
    if request.method == 'GET':
        try:
            ...
            print(f"遇到错误：{e}")
            return JsonResponse({"code": 12094, "msg": "遇到错误"})
...
def pub_upload_records(request):
    if request.method == 'GET':
        try:
            ...
            records = [{
                "username": record.uid.username,
                'upload_time': record.createTime.strftime('%Y-%m-%d %H:%M:%S'),
                "file_name": record.file_name,
                "status": record.status,
...
def share_operate(request):
    if request.method == 'PATCH':
        try:
            ...
            print(f"遇到错误：{e}")
            return JsonResponse({"code": 12093, "msg": "遇到错误"})
...
def partition_base_info(request):
    if request.method == 'GET':
        try:
            ...
            print(f"遇到错误：{e}")
            return JsonResponse({"code": 12092, "msg": "遇到错误"})
...
def pub_partition_info(request):
    if request.method == 'GET':
        try:
            ...
            print(f"遇到错误：{e}")
            return JsonResponse({"code": 12091, "msg": "遇到错误"})
...
def admin_file_records(request):
    if request.method == 'GET':
        try:
            ...
            print(f"遇到错误：{e}")
            return JsonResponse({"code": 12090, "msg": "遇到错误"})
...
def psl_pid_filerecord(request):
    if request.method == "GET":
        try:
            ...
            print(f"遇到错误：{e}")
            return JsonResponse({"code": 12089, "msg": "遇到错误"})
...
def pub_pid_filerecord(request):
    if request.method == "GET":
        try:
            ...
            print(f"遇到错误：{e}")
            return JsonResponse({"code": 12088, "msg": "遇到错误"})
...
def rename_kbase(request):
    if request.method == 'PATCH':
        try:
            ...
            print(f"遇到错误：{e}")
            return JsonResponse({"code": 12087, "msg": "遇到错误"})
    ...
    if request.method == "DELETE":
        try:
            ...
            if ktype == 'personal':
                ...
                subprocess.Popen(['python', '/data/ssl_chat/ssl_model/kDataBase/fileDel_shell.py'] + params)
            ...
            print(f"遇到错误：{e}")
            return JsonResponse({"code": 12086, "msg": "遇到错误"})
    ...
    if request.method == "DELETE":
        try:
            ...
            if ktype == 'personal':
                ...
                subprocess.Popen(['python', '/data/ssl_chat/ssl_model/kDataBase/partitionDel_shell.py'] + params)
            ...
            print(f"遇到错误：{e}")
            return JsonResponse({"code": 12084, "msg": "遇到错误"})